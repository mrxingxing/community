...
public class EventConsumer implements CommunityConstant {
    ...
    //加精和置顶通知，待开发
    /*
    @KafkaListener(topics = {TOPIC_WONDERFUL,TOPIC_UNWONDERFUL})
    public void handleWonderfulMessage(ConsumerRecord record){
        if(record==null||record.value()==null){
            logger.error("消息的内容为空");
            return;
        }
    ...
    @KafkaListener(topics = {TOPIC_UNTOP,TOPIC_TOP})
    public void handleTopMessage(ConsumerRecord record){
        if(record==null||record.value()==null){
            logger.error("消息的内容为空");
            return;
        }
